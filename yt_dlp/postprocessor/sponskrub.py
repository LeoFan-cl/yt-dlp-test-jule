...
class SponSkrubPP(PostProcessor):
    ...
    def __init__(self, downloader, path=u'', args=None, ignoreerror=False, cut=False, force=False, _from_cli=False):
        ...
        if not ignoreerror and self.path is None:
            if path:
                raise PostProcessingError(u'sponskrub not found in "%s"' % path)
            else:
    ...
    def run(self, information):
        ...
        self.to_screen(u'Trying to %s sponsor sections' % (u'remove' if self.cutout else u'mark'))
        if self.cutout:
        ...
        self.write_debug(u'sponskrub command line: %s' % shell_quote(cmd))
        stdout, _, returncode = Popen.run(cmd, text=True, stdout=None if self.get_param(u'verbose') else subprocess.PIPE)

        if not returncode:
            os.replace(temp_filename, filename)
            self.to_screen(u'Sponsor sections have been %s' % (u'removed' if self.cutout else u'marked'))
        elif returncode == 3:
            ...
            raise PostProcessingError(
                ...
                or u'sponskrub failed with error code %d' % returncode)
        return [], information
